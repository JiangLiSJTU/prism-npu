"""Qwen2.5-0.5B prefill ONNX export — SDPA path (Issue #3 Phase 3).

Outputs: models/qwen2_5_05b_prefill_S{S}_b{B}_sdpa.onnx
"""
import argparse, sys
...
def export(S, batch, output_dir):
    print(f"\n=== Qwen2.5-0.5B prefill SDPA export (S={S} b={batch}) ===")
    from transformers import AutoModelForCausalLM
    _patch_qwen3_masking(); _patch_torch_diff(); _patch_cumsum()
    print(f"  loading Qwen/Qwen2.5-0.5B with attn_implementation='sdpa' ...")
    ...
    out = output_dir / f"qwen2_5_05b_prefill_S{S}_b{batch}_sdpa.onnx"
