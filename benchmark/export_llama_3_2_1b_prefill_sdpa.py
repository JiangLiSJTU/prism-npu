"""Llama-3.2-1B prefill ONNX export — SDPA path (Issue #3 Phase 3).

Outputs: models/llama_3_2_1b_prefill_S{S}_b{B}_sdpa.onnx
"""
import argparse, sys
...
LLAMA_IDS = ["unsloth/Llama-3.2-1B", "meta-llama/Llama-3.2-1B", "NousResearch/Llama-3.2-1B"]
...
def load_llama():
    ...
    for mid in LLAMA_IDS:
        try:
            print(f"  trying {mid} ...")
            m = AutoModelForCausalLM.from_pretrained(
                mid, attn_implementation="sdpa", torch_dtype=torch.float32)
            print(f"  ✓ loaded {mid}")
            return mid, m
        except Exception as e:
...
def export(S, batch, output_dir):
    print(f"\n=== Llama-3.2-1B prefill SDPA export (S={S} b={batch}) ===")
    _patch_qwen3_masking(); _patch_torch_diff(); _patch_cumsum()
    mid, base = load_llama()
    if base is None:
        print("  ✗✗ no Llama mirror accessible")
        return False
    base.eval()
    ...
    out = output_dir / f"llama_3_2_1b_prefill_S{S}_b{batch}_sdpa.onnx"
